...
/** Return the most recent task-tool result matching `toolName` in the session transcript. */
export function reconstructTasks(entries: readonly SessionEntryLike[], toolName: string): Task[] {
  const match = entries.findLast((entry) => entryToolName(entry) === toolName);
  if (!match) return [];
  const details = entryDetails(match);
